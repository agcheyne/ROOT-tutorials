...
void tut11()
{
	...
	//adding a line on the canvas -- for example to show a baseline
	TLine *l = new TLine(0, 20, 13, 20); // TLine(minx ,miny ,maxx ,maxy)
	l->SetLineWidth(2);
	l->SetLineColor(kOrange);
	l->Draw();
	
	// adding arrows to highlight some data
	double x0 = 6.3;
	int bin = hist->FindBin(x0); //finding the bin for variable
	double y0 = hist->GetBinContent(bin);
	
	TArrow *arr = new TArrow(8, 30, x0, y0); //(xstart,ystart,xend,yend)
	arr->SetLineWidth(2);
	arr->SetArrowSize(0.02); //default is 0.05
